class TeacherCoursesController < ApplicationController
  before_action :set_teacher_course, only: %i[ show edit update destroy ]

  # GET /teacher_courses or /teacher_courses.json
  def index
    @teacher_courses = TeacherCourse.all
  end

  # GET /teacher_courses/1 or /teacher_courses/1.json
  def show
  end
  ...
  # POST /teacher_courses or /teacher_courses.json
  def create
    @teacher_course = TeacherCourse.new(teacher_course_params)

    respond_to do |format|
      if @teacher_course.save
        format.html { redirect_to teacher_course_url(@teacher_course), notice: "Teacher course was successfully created." }
  ...
  # PATCH/PUT /teacher_courses/1 or /teacher_courses/1.json
  def update
    respond_to do |format|
      if @teacher_course.update(teacher_course_params)
        format.html { redirect_to teacher_course_url(@teacher_course), notice: "Teacher course was successfully updated." }
  ...
  # DELETE /teacher_courses/1 or /teacher_courses/1.json
  def destroy
